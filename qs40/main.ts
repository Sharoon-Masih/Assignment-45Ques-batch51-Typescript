...
function make_album(artist_name:string,album_title:string,) 
{
    const music_album:{
        ...
        artistName:artist_name.split(" ").map((up:string)=>up.charAt(0).toUpperCase()+up.slice(1)).join(" "),
...
function make_Album(artist_name:string,album_title:string,NumberOfTrack?:number) 
{
if (NumberOfTrack){
    const music_album:{
        artistName:string[] | string;
